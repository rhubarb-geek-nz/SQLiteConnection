...
foreach ($SRC in @($SQLZIP, $SQLURL, "$PublishDir/sqlite-netStandard20-binary"), @($WINZIP, $WINURL, $PublishDir))
{
	$ZIP = $SRC[0]
	$URL = $SRC[1]
	$DEST = $SRC[2]
...
{
	if (Test-Path "$PublishDir/runtimes/win-$A/native/$SQLINTEROP")
	{
		$null = New-Item -Path $PublishDir -Name $A -ItemType 'directory'

		$null = Move-Item -Path "$PublishDir/runtimes/win-$A/native/$SQLINTEROP" -Destination "$PublishDir$A/$SQLINTEROP"
	}
}
